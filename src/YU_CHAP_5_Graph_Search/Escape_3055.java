...
public class Escape_3055 {
    ...
    static int[][] dir = {{1, 0}, {0, 1}, {-1, 0}, {0, -1}}; //상하좌우 방향 배열
    ...
    static void bfs_water() {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {  //모든물을 다 넣어줘야 함 
                dist_water[i][j] = -1;
                visit[i][j] = false;
                if (a[i].charAt(j) == '*') {
                    Q.add(i);
                    Q.add(j);
                    dist_water[i][j] = 0; //물 탐색 성공 
                    visit[i][j] = true;
                }
            }
        }
        ...
        while (!Q.isEmpty()) {
            int x = Q.poll();
            int y = Q.poll();
            for (int k = 0; k < 4; k++) { //상하좌우 =>4 
                int nx = x + dir[k][0], ny = y + dir[k][1];
                if (nx < 0 || ny < 0 || nx >= N || ny >= M) continue;  // 지도를 벗어나는 곳으로 가는가?
                if (a[nx].charAt(ny) != '.') continue;  // 갈 수 있는 칸인지 확인해야 한다. .은 벽이므로 패스 
